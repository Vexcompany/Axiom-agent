...
/** Upstream Gemini default — 2.5-flash no longer available to new users. */
...
function resolveProvider(
  ...
  if (env.SENSENOVA_API_KEY && lower.startsWith("sensenova/")) {
    return {
      ...
      config: {
        id: "sensenova",
        baseUrl: env.SENSENOVA_BASE_URL || "https://token.sensenova.ai/v1",
        apiKey: env.SENSENOVA_API_KEY,
      },
      resolvedModel: model.replace(/^sensenova\//i, ""),
    };
  }
